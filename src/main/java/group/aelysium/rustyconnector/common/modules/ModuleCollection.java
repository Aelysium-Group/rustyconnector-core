...
public class ModuleCollection implements Closure, ModuleHolder {
    protected final Map<String, Particle.Flux<?>> modules = new ConcurrentHashMap<>();
     ...
     */
    public void registerModule(ModuleTinder<?> module) throws Exception {
     ...
     */
    public void registerModule(@NotNull String key, ModuleTinder<?> module) throws Exception {
        Particle.Flux<?> flux = module.flux();
        ...
        flux.observe(1, TimeUnit.MINUTES);
     ...
     */
    public <T extends Particle> Particle.Flux<T> fetchModule(String name) {
    ...
    @Override
    public Map<String, Particle.Flux<? extends Particle>> modules() {
